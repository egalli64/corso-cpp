...
int main() {
    ...
    std::thread t1{ adder_function, std::cref(values), std::ref(r_1) };
    std::thread t2{ Adder(values, r_2) };
    std::thread t3{ [&] { r_3 = std::accumulate(values.begin(), values.end(), 0); } };
